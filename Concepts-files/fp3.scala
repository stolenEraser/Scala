...
object fp3 {
  ...
  def member (a : Int, xs : List[Int]) : Boolean = {
    ...
    xs match{
      case Nil=> false;
      case x::xs=> if (x == a)true else member(a,xs);
    }
  }  
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    ...
    xs match{
      case Nil=> true;
      case  x::Nil=> true; //last item in the list
      case x::xs => if (x== xs.head) allEqual(xs.tail) else false;
     }
  }
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = {
    ...
    ys match{
      case Nil=> ys;
      case z::zs => if(z== x) ys.filterNot(x=> x == z) else delete1(x, zs);
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def removeDupesAux [X] (x:X, num:Int,xs:List[X]) :List[(Int,X)] = {
	    xs match{
	      case Nil => if (x==Nil) List() else (num,x)::List()
	      case y::ys => if(y == x) removeDupesAux(x,num+1,ys) else (num,x)::removeDupesAux(y,1,ys)
      }
    }
	  if(xs.isEmpty != true) removeDupesAux(xs.head,1,xs.tail)
	  else List()
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    ...
    (n, xs) match{
      case (-1, xs)=> (List[X](), xs);
      case (_, Nil)=> (List[X](), List[X]());
      case (n,x:: xs)=> {
        val(l1,l2)= splitAt(n-1, xs)
        if (n > 0 ) {
          (x::l1,l2)
        } else{
          (l1,x::l2);
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
    ...
    xs match{
      case Nil => true;
      case x::xs => if (member(x,xs)) false else allDistinct(xs);
    }
  }
  allDistinct (List (1,2,3,2,4,5));
