...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def af(x: String, z:String): String= {x + z + term};
    xs match{
      case Nil => "";
      case t::ts => foldLeft(ts, af("",t), af);
    }    
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    // TODO: Provide definition here.
    def af(x: String, z:String): String= {delimiter + x+ z};
    xs match{
      case  Nil => "";
      case  y::ys => y+ foldRight(ys,delimiter, af);
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    def thereIsGreater( a:Int, idx:Int, xs:List[Int]) :Int= {
      xs match{
        case Nil=> throw new RuntimeException();
        case y::ys => if(y >= a) idx else thereIsGreater(a,idx+1,ys);
      }
    }
    thereIsGreater(a,0,xs);
