...
object fp1 {
  ...
  // hardcoding the numbers 1,2,4,6,24,120.
  ...
  def sumTailRecursive (xs : List[Int]) : Int = {
    ...
    return sumTailRecursiveAux(pointer, xs);
  }
  ...
  def maxTailRecursiveAux (accumulator : Int, xs : List[Int]) : Int = {
    ...
    if(xs.isEmpty){
      ...
      val maxVal: Int= if (xs.head > accumulator) xs.head else accumulator
      maxTailRecursiveAux(maxVal, xs.tail);
    }
  }
