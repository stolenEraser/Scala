...
object argpass {
  ...
  def refint2 (f : RefInt => Unit) : (Int, Int, Int) = {
    ...
    //create 3 instances of RefInt
    val ref1= new RefInt(20);
    f(ref1);
    val ref2= new RefInt(10);
    f(ref2);
    val ref3= new RefInt(30);
    f(ref3);

    (ref1.get(), ref2.get(), ref3.get())
  }
  ...
  def refint3 (r : RefInt) : RefInt = {
    ...
    val addTo= val1+ 1;
    r.set(addTo);
    //return double the original value
    new RefInt(val1*2);
  }

  // EXERCISE 4: complete the following function.
  // It has two parameters
  // - r: (a reference to) an instance of RefInt (see above for the definition of the class RefInt); 
  //AND
  // - f: a function that accepts (a reference to) an instance of RefInt.
  // Your code must call f with a *copy* of the RefInt r, i.e., 
  //it must make a new instance of RefInt with the same Int.
  ...
  def refint4 (r : RefInt, f : RefInt => Unit) : Boolean = {
    // TODO: Provide definition here.
    val newInstance= new RefInt(r.get());
    f(newInstance)
    val fCopy= newInstance.get()
